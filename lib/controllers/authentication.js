...
authenticationController.register = async function (req, res) {
  ...
  try {
    ...
    if (!userData['account-type'] || userData['account-type'] !== 'student' && userData['account-type'] !== 'instructor' && userData['account-type'] !== 'TA') {
...
authenticationController.onSuccessfulLogin = async function (req, uid) {
  ...
  try {
    ...
    }), user.auth.addSession(uid, req.sessionID), user.updateLastOnlineTime(uid), user.updateOnlineUsers(uid), analytics.increment('logins'), db.incrObjectFieldBy('global', 'loginCount', 1)]);
    if (uid > 0) {
